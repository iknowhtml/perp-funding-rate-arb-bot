...
describe("createRequestPolicy", () => {
  ...
  describe("rate limiting", () => {
    ...
    it("should use correct bucket based on endpoint category", () => {
      ...
      // COINBASE_RATE_LIMITS preset uses same bucket (10 tokens) for all categories
      expect(policy.getAvailableTokens("/api/ticker")).toBe(10);
      expect(policy.getAvailableTokens("/api/account")).toBe(10);
      expect(policy.getAvailableTokens("/api/order")).toBe(10);
    });
  });
