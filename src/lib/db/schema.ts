...
export const executionEstimate = pgTable(
  "execution_estimate",
  {
    ...
    timestamp: timestamp({ withTimezone: true }).notNull(),
    market: text().notNull(),
    positionSizeUsd: numeric({ precision: NUMERIC_PRECISION, scale: 0, mode: "bigint" }).notNull(),
    ...
    acceptablePriceUsd: numeric({ precision: NUMERIC_PRECISION, scale: 0, mode: "bigint" }),
    createdAt: timestamp({ withTimezone: true }).defaultNow(),
  },
